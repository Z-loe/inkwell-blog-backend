...
public class InitSqlite {
    ...
    public InitSqlite() throws SQLException, ClassNotFoundException {
        ...
        String sqlCreateUserTable = "create table if not exists User(uid text, nickname text, account text, password text, userType int, token text, avatar text)";
        String sqlCreateCategoryTable = "create table if not exists Category(id text, name text)";
        String sqlCreateArticleTable = "create table if not exists Article(id text, title text, content text, categoryId text, createTime text, comment text)";
